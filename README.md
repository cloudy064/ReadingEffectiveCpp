...
也许你并不需要关心什么客户端，但是我愿意花上一点时间让你与客户端能够和谐共处。毕竟你对于其它程序员来说，也可能成为客户端。难道你不希望别人为你提供简单有效的东西吗？此外，你还可能称为你自己代码的客户端，那个时候你就会为你开发接口时关注客户端而高兴了。

本书中，对函数和函数模板、类和类模板的差异，我将一带而过。因为两者的规则其实大部分都是通用的。当出现不通用的情况时，我会特别对他们进行区分。

当在代码注释中指出构造函数和析构函数时，我往往会使用缩写`ctor`和`dtor`。

## 命名规定
我已经努力为对象、类、函数、模板等取一个有意义的名字了，但是其中某些名字的含义可能不会那么明显。比如，有两个我最喜欢的名字，`rhs`和`lhs`。它们分别表示“left-hand side(左边)”和“right-hand side(右边)”。我经常用它们作为二元操作符函数的名字，即`operator==`和`operator*`等。比如，如果`a`和`b`是两个用来表示分数的数字，并且`Rational`对象可以通过`operator*`做乘法，那么下面的表达式：
```cpp
a * b
```
就等价于
...
operator*(a, b);
```
在第24项中，我这样来声明`operator*`：
```cpp
const Rational operator*(const Rational& lhs, const Rational& rhs);
```
正如你所看到的，左边的操作数是`a`，也就是`lhs`，又变的操作数是`b`，也就是`rhs`。

对于成员函数来说，左边的参数被`this`指针代替了，所以有时我会单独使用`rhs`。你可能已经在前面见过这种情况了，比如前文的`Widget`成员函数。我经常在文中使用`Widget`类作为例子，“Widget”只是一个没有任何意义的名字而已，和GUI组件里面的控件没有任何关系。

我经常这样来命名指针，如果是指向类型T的指针，我取名为pt(pointer to T)，下面是一些例子：
```cpp
Widget *pw; // pw = ptr to Widget

class Airplan;
Airplan *pa;    //pa = ptr to Airplan

class GameCharacter;
GameCharacter* pgc; //pgc = ptr to GameCharacter
```
引用(reference)也是一样的命名方式：`rw`就是指向类型`Widget`的引用，而`ra`就是`Airplane`的引用。

我偶尔也会使用`mf`来作为成员函数(member function)的名字。

## 对线程的考虑
作为一门语言，C++并没有任何线程的概念（新C++标准中已有），其实连任何并发的概念都没有。C++标准库也同样没有这些概念。就C++来看，多线程程序并不存在。

然而它们其实确实存在。虽然我的关注点集中于标准C++，但是我也无法忽略线程安全是很多程序需要面对的一个问题。对于这种标准C++和实际应用程序之间的分歧，我的处理方式是指出C++可能会导致多线程问题的地方。本书并不密切关注多线程编程，相反，本书大部分情况下只会关注单线程下的C++编程，而对于多线程编程，本书只会指出在采用我的建议时，你需要特别关注哪些东西。

如果你对多线程编程不熟悉，也不用担心，你可以忽略我对多线程相关的一些附注。但是，如果你目前在编写多线程程序或者程序库，那么我的附注不过是你学习使用C++需要关注的问题的冰山一角。

## TR1和Boost库
你会在本书中看到很多TR1和Boost库的引用。每个库都有一个专门的条目来详细描述，但是，它们不幸被放在了本书的最后。当然如果你喜欢，你可以现在就去阅读它们，但是如果你更喜欢从头到位阅读本书的话，下面的摘要也许对你有用。
* TR1("Technical Report 1")是一个对C++标准库中的新函数对象的一个特化。这些函数接受类模板和函数模板作为参数，比如哈希表，计数型智能指针，正则表达式等等。TR1中所有的内容都在`tr1`命名空间中，同时`tr1`命名空间又位于`std`命名空间中。
* Boost是一个组织，建立了一个网站，主要提供可移植的、经过评审的C++开源库。TR1的大多数功能都是基于Boost的，直到编译器厂商在C++库中包含了TR1之前，开发者如果想要了解TR1的实现，大多数都会到Boost网站上去看。Boost提供了比TR1多得多的功能，所以无论如何，它都是值的去掌握的。
